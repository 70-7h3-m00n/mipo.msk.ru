...
const sendLeadMagnitRequest = async (click_id, email) => {
  try {
...
const hitContactRoute = async (values) => {
  try {
    ...
    if (output === 200 && values.click_id && values.utms.utm_campaign == 'sravniru') {
      await sendSravniRequest(values.click_id);
    }

    if (output === 200 && values.utms.utm_source == 'leadmagnet' && values.utms.utm_term) {
