...
const PagesProgram = ({ ofType = null, reviews }: PagesProgramType) => {
  ...
  useEffect(() => {
    ...
    const foundOtherTariff = ['Marketing', 'jekonomika', 'Management', 'Jurisprudence', 'nutriciologiya'].find(field => {
      if (field === program.study_field?.slug) {
          ...
          desc={
            <>
              ...
              <span className={stls.highlightFw}>
                со скидкой <span className={stls.highlightC}>{SALE_VALUE}%</span>
