    ...
    <div className={styles.header}>
      <div className={styles.topContent}>
        ...
        <div className={styles.burgerWrapper}>
          <svg style={{ display: openMenu ? 'none' : 'flex' }} onClick={() => setOpenMenu(true)} width="60" height="60"
          ...
          <div className={styles.wrapperSvg} style={{ display: !openMenu ? 'none' : 'flex' }}>
      ...
      <div className={styles.bottomContent} style={{ display: !openMenu ? 'none' : '' }}>
        <nav className={styles.navList}>
          <div className={styles.listProgram}>
            ...
            >
              <Link href={'/catalog'} className={styles.link}>ВСЕ НАПРАВЛЕНИЯ</Link>
              {
                facultyData?.filter(item => validFilter
                  ?.some(elem => item.title === elem.faculty.title))
                  ?.map((item, index) => (
                    <Link href={'/catalog/' + item.slug} key={index} className={styles.link}>{item.title}</Link>
                  ))
              }
            </div>
          </div>
          {
            MenuAboutInstitute.map((item, index) => (
              <Link className={styles.linkHeader} href={item.link} key={index}>{item.text}</Link>
            ))
          }
        </nav>
