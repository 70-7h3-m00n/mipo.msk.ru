...
export const Slider = ({ children, stylesBtn, stylesList }: Props) => {
  ...
  const onScrollSlider = (position: 'left' | 'right') => {
    ...
    if (position === 'right') {
      const pivotRight =
        listSlider.current!.scrollWidth - parentWidth.current!.clientWidth;
  ...
  const handleToggleList = (e: SyntheticEvent<HTMLDivElement>): void => {
    const touchPoint: number =
      ...
      <div className={classNames(styles.wrapperBtn, stylesBtn)}>
        <button className={styles.btn} onClick={() => onScrollSlider('left')}>
          ...
          >
            ...
            <path
              d="M32.206 30L33.7939 28.4335L25.339 20L33.7939 11.5665L32.206 10L22.206 20L32.206 30Z"
              fill="black"
            />
          </svg>
        </button>
        ...
        <button className={styles.btn} onClick={() => onScrollSlider('right')}>
          ...
          >
            <rect width="60" height="40" rx="10" fill="white" />
            <path
              d="M27.794 30L26.2061 28.4335L34.661 20L26.2061 11.5665L27.794 10L37.794 20L27.794 30Z"
              fill="black"
            />
          </svg>
        </button>
