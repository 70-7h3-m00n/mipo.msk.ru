...
export const PopUpContentProgram = ({ isFaculty }: PopUpContentProgram) => {
  ...
  const data = coursesData.filter(
    item => item.faculty.title === isFaculty && item.popularCourses
  );
  ...
  const isPathCourse = (slug: string) => {
    const typeProgramSlug = Object.fromEntries(
      typeProgramData.map(item => [item.title, item.slug])
    );
    const facultySlug = Object.fromEntries(
      facultyData.map(item => [item.title, item.slug])
    );
    ...
    <div className={styles.container}>
      <div className={styles.block}>
        ...
        {data.length > counterItem && (
          <ButtonLink
            text={counter === counterItem ? 'смотреть все' : 'скрыть'}
            onClick={toggle}
            active={true}
            styleOption={'square'}
          />
