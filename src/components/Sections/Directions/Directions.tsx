...
import LinkProfession from '@/components/LinkProfession';
...
export const Directions = () => {
  ...
  const onDataPath = useCallback(() => {
    ...
    const typeProgramMap = Object.fromEntries(
      typeProgramData?.map(tp => [tp.title, tp.slug])
    );

    return (coursesData as Array<IProgramCourses>).reduce(
      (acc: Array<DataPath>, item) => {
        if (
          !acc.some(
            entry =>
              entry.faculty.title === item.faculty.title &&
              entry.typeProgram.title === item.typeProgram.title
          )
        ) {
          acc.push({
            faculty: {
  ...
  const filterTypeProgram = () =>
    typeProgramData?.filter(item =>
      onDataPath()?.some(elem => item.title === elem.typeProgram.title)
    );

  const [filterProgram, setFilterProgram] = useState(
    filterTypeProgram()[activeProgram]?.title
  );

  const filterFaculty = () =>
    onDataPath().filter(item => item.typeProgram.title === filterProgram);
      ...
      <div className={styles.content}>
        ...
        <div className={styles.profession}>
          {filterFaculty()?.map((item, i) => (
            <LinkProfession
              href={`/catalog/${item!.typeProgram.slug}/${item!.faculty.slug}`}
