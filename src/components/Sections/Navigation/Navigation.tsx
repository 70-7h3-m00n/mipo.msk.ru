...
import CardCourse from '../../Items/CardCourse'
...
export const Navigation = () => {
  ...
  const typeProgram = useAppSelector(state => state.typeProgramReducer.typeProgramData)
  ...
  const [validFilter, setValidFilter] = useState<Array<IValidFilter> | null>(null)
  ...
  const typeProgramCourse = (): Awaited<ReturnType<typeof fetchTypeProgram>> => {
    return typeProgram?.filter((item) => item.slug === routerPath(1)
      || facultySlug.includes(routerPath(1))
      || routerPath(1) === ''
    )
  }

  const filterCourse = (title: string): Awaited<ReturnType<typeof fetchProgramCourses>> => {
    return courses.filter(courseItem => courseItem.typeProgram.title === title)
  ...
  useEffect(() => {
    ...
    if (dataPath.length === 2) {
      if (!faculty.some(item => item.slug === dataPath[1])) {
        return setValidFilter(mapData.filter(item => item.typeProgram.slug === dataPath[1]))
      }
    }
  ...
  return (
    <section className={classNames('container', styles.section)}>
      <div className={styles.headerWrapper}>
        <h2 className={styles.header}>Направления <br/> обучения</h2>
      ...
      <div className={styles.wrapperBtnPrograms}>
        <div className={styles.listBtnProgram}>
          <ButtonLink active={router.asPath === '/catalog' || typeProgram.some(item => lastPathData.includes(item.slug))}
                      hrefLink={hrefLinkFacultyAll()}
                      text={'Все направления'}
                      styleOption={'round'}
          />
          {
            faculty?.filter(item => validFilter
              ?.some(elem => item.title === elem.faculty.title))
              .map((item, index) => (
                <ButtonLink  hrefLink={hrefLinkFaculty(item.slug)}
        ...
        <div className={styles.mobileBlock}>
          <Select headerSelect={'выбрать направление'}
          ...
          >
            {
              faculty?.filter(item => validFilter
                ?.some(elem => item.title === elem.faculty.title))
                .map((item, index) => (
                  <SelectOption onClick={() => setFacultySelect(item.title)}
        ...
        <div className={styles.listBtnTypeProgram}>
          ...
          >
            <button onClick={() => setOpenTypeProgram(!openTypeProgram)} className={styles.programBtn}>
            ...
            <button className={styles.mobileBlock} onClick={() => setOpenSearchMobile(true)}>
              <MagnifierSvg />
            </button>

            <InputSearch classNames={styles.inputSearchWrapper}
              ...
              <div className={styles.filterTypeProgram}>
                ...
                </span>
                {
                  typeProgram?.map((item, index) => (
                    <div onClick={() => router.push(`/catalog/${item.slug}`, '', {
                      scroll: false,
                    })}
            ...
            <Fragment key={i}>
              {
                filterCourse(typeCourse.title).length !== 0 ?
                  <div className={styles.wrapperProgramCourse}>
                    <h3 className={classNames('header', styles.headerProgram)}>{typeCourse.title}</h3>

                    <div className={styles.listCourseItem}>
                      {
                        filterCourse(typeCourse.title).map((course, index) => (
                          <CardCourse srcIcon={faculty!.find((elem) =>
                                        elem.title === course.faculty.title)!.icon[0].url
                                      }
