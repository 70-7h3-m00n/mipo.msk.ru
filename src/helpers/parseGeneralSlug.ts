export const parseGeneralSlug = (str: string | null | undefined) => {
  ...
  const isProgramsPage =
    (strSplit &&
      [...higherEducationKeys, ...courseKeys, ...programsKeys].some(
        key => key === strSplit?.[0]
      )) ||
    false;
  ...
  const isProgramPage =
    (!isProgramsPage &&
      [...higherEducationKeys, ...courseKeys].some(
        key => key === strSplit?.[strSplit.length - 1]
      )) ||
    false;
  ...
  const currentJournalsByFacultySlug =
    (isJournalsPage && strSplit?.[1]) || null;
