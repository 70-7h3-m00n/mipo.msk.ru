  ...
  const paths = (
    await Promise.all(
      ctx.locales!.map(async () => {
        ...
        const slugTypeProgram = courses.reduce(
          (previousValue, currentValue) => {
            if (typeProgramMap[currentValue.typeProgram.title]) {
              return previousValue.add(
                typeProgramMap[currentValue.typeProgram.title]
              );
            }
            return previousValue;
          },
        ...
        const secondRowUrl = courses.reduce((previousValue, currentValue) => {
          if (
            typeProgramMap[currentValue.typeProgram.title] &&
            typeProgramMap[currentValue.faculty.title]
          ) {
            return previousValue.add(
              `${typeProgramMap[currentValue.typeProgram.title]}/${typeProgramMap[currentValue.faculty.title]}`
            );
          }
          return previousValue;
        }, new Set(''));
        ...
        const slugCourses = courses.reduce((previousValue, currentValue) => {
          if (
            ...
            return previousValue.add(
              `${typeProgramMap[currentValue.typeProgram.title]}/${typeProgramMap[currentValue.faculty.title]}/${currentValue.slug}`
            );
          }
          return previousValue;
        }, new Set(''));
        ...
        const catalogPathUrl = [
          ...
          return previousValue.add(
            `https://mipo.msk.ru/catalog/${currentValue}`
          );
        }, new Set(''));
        //===============================================
